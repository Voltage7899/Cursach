...
<div>
	<div class="basket">
		<header class="bas">Your purchases:</header>
		
		<?php
		$name=$_COOKIE['user'];
		$result=mysqli_query($connection,"SELECT * FROM `basket` WHERE `nameofuser`='$name' ");
		?>
		<ul>
			<?php
		if($name=='')
		...
		{
			echo '<li>'."Name: ".' '.$count['nameofthing'].' '."Total:".$count['totalprice'].'<br>';
		}
		...
		$result=mysqli_query($connection,"DELETE FROM `basket` WHERE `nameofuser`='$name'");
		?>
